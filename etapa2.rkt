...
(define (get-ch-words words ch)
  ...
  (filter (lambda (word) (and (not (null? word)) (char=? (car word) ch))) words)
...
(define (ast-func suffixes)
  ...
  (cons (list (car (car suffixes))) (map (lambda (word) (cdr word)) suffixes))
...
(define (cst-func suffixes)
  ...
  (define prefix (longest-common-prefix-of-list suffixes))
  (cons prefix
        (map (lambda (word)
               (cadr (longest-common-prefix word prefix))) suffixes)
  )
)
...
(define (suffixes->st labeling-func suffixes alphabet)
  ...
  (define initial-list-words (map (lambda (ch) (get-ch-words suffixes ch)) alphabet)) ; sortez listele care incep cu acelasi caracter

  (define listwords (filter (lambda (x) (not (null? x))) initial-list-words)) ; scap de listele vide
 
   (map (lambda (lists)
         (if (null? lists)
             '()
             (let* (
                    (label (car (labeling-func lists))) ; eticheta
                    (suffixes (cdr (labeling-func lists))) ; sufixele
                    )
               (append (list label) (suffixes->st labeling-func suffixes alphabet))
             ; construiesc o lista cu eticheta listei si apelez recursiv functia cu restul elementelor din lista fara eticheta
             )
         )
    ) listwords) 
...
(define text->st
  ...
  (lambda (labeling-func)
    ...
    (lambda (word)
           ( let* (
            (alphabet (sort (remove-duplicates (append word (list #\$))) char<?)) ; gasesc toate caracterele cuvantului, inclusiv $ si le sortez
            (suffixes (get-suffixes (append word (list #\$)))) ; construiesc lista de sufixe
            )
